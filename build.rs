...
fn compile_resource(out_dir: &str) {
    ...
    Command::new(rc_path.as_ref().map_or(Path::new("rc.exe"), Path::new))
        .args(&["/fo", &format!("{}/http-manifest.lib", out_dir), "http-manifest.rc"])
        .status()
...
fn compile_resource(out_dir: &str) {
    Command::new("windres")
        .args(&["--input", "http-manifest.rc", "--output-format=coff", "--output"])
        .arg(&format!("{}/http-manifest.res", out_dir))
        .status()
        .unwrap();

    Command::new("ar")
        .args(&["crs", "libhttp-manifest.a", "http-manifest.res"])
